...
def remember(memory, experience, over, max_memory = 100):
    ...
    if memory.shape[0] == 1:
        memory = new_experience.reshape(1, memory.shape[1])
...
def get_one_batch(memory, model, batch_size = 10, max_memory = 100, grid_size = 10, discount = 0.9):
    if memory.shape[0] < max_memory:
        mem_rows = mem.shape[0]
    else:
        mem_rows = max_memory
    n_actions = model.output.shape[-1]
    canvas_size = grid_size **2
    inputs = np.zeros((min(len_memory, batch_size), canvas_size))
    target = np.zeros((inputs.shape[0], n_actions))
    i = np.random.choice(n.arange(mem_rows), inputs.shape[0])
    ...
    # update inputs
    inputs[i] = state_vectors[0]
    inputs[i+1] = state_vectors[0]
